...
public class ScoreBoardBingo : MonoBehaviour
{
    ...
    void Start()
    {
        nombreColonnes = 2; // Nombre de colonnes de votre leaderboard
        filePath = Path.Combine(Application.dataPath, "StreamingAssets/Leaderboard/leaderboardBingo.json"); // Définition du chemin du fichier
    ...
    public void triLeaderboard(string type)
    {
        ...
        if (loadedData["history"].Count != 0)
        {
            ...
            {
                case "name":
                    g = GameObject.Find("name").transform.GetChild(0).transform;
                ...
                case "score":
                    ...
                    GameObject.Find("name").transform.GetChild(0).transform.rotation = new Quaternion(0, 0, 0, 0);
                    comparer = new Array2DSort(history, 0);
                    comparer = new Array2DSort(history, 1);
                    break;
                default:
